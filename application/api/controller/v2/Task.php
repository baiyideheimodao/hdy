...
class Task extends WeChatFactory {
    ...
    private function typeTaskListOfHome(){
        ...
        $class_ids = db('type_task_log_table')->where('class_id',$class_id)->select();
        $class_ids = array_column($class_ids,'type_task_id');
        ...
        function($v)use($class_ids){
            return in_array($v['id'],$class_ids);
        });
    ...
    private function typeTaskList($type = self::TYPE_SCHO){
        ...
        foreach ($list as $key => $item){
            ...
            // 记录题，需判断是否中断
            // 获取阶段任务未完成记录第一条
            $firstInfo = db('user_type_task_log_table uttlt')
                ->join('task t','t.id = uttlt.task_id','left')
                ->field('uttlt.*,t.type task_type')
                ->where('uttlt.user_type_task_id',$user_type_id)
            ...
            if($firstInfo['done_time'] < strtotime(date('Y-m-d'))&&$firstInfo['task_type']==2){
                // 判断执行日期是否小于当前日期，则将状态设置为【-4已中断】
                $list[$key]['type_status'] = -4;
            }
        }
    ...
    public function taskList(){
        ...
        switch ($type){
            ...
            case self::TYPE_SOFT:
                // 校内轻课任务列表
                return $this->softTaskList($type);
                break;
            case self::TYPE_HAPPY:
                return $this->getHappyVillage();
            default:
    ...
    //获取村庄
    public function getHappyVillage(){
        $params = $this->request->get();
        $class_id = $params['class_id'];
        $status = db('userBase')->where('user_id',$params['user_id'])->find();
        // var_dump($params);
        if(empty($status)){
            $status = [
            'b0'=>0,
        ...
        $list = array_map(function($v){
            $num = count(db('user_enroll')->where('base_id',$v['id'])->select());
            $v['mark'] = "已经有 $num 人报名";
            return $v;
        },getHappyVillage($status,$class_id)());
        $data = [
            'list'=>array_values($list),
            'totalPage'=>1,
            'currentPage'=>1
        ];
        return json(['data'=>$data,'code'=>1,'msg'=>'获取成功'],200);
    }

    // 项目列表
    public function newstaskList(){
        if(empty($this->info)){
            return $this->returnJson([],'请先登录');
        }
        $page  = $this->request->param('page', 1);
        ...
        if($qktotal>0){
            ...
            if(!$is_true){
                $list = db('type_task_table')
                    ...
                    ->whereIn('ttlt.class_id',$this->info['class_ids'])
                    ->limit(($page-1)*$limit,$limit)
                    ->field("t.id task_id,ttt.id,ttt.`mode`,uttt.id user_type_id,if(instr(ttt.image,'http')>0,ttt.image,concat('{$this->webUrl}',ttt.image)) image,ttt.title,ttt.depict,ttt.type,FROM_UNIXTIME(ttt.start_date,\"%Y-%m-%d\") start_date,if((uttt.task_over_time is null or uttt.task_over_time = 0),'',FROM_UNIXTIME(uttt.task_over_time,\"%Y-%m-%d\")) end_date,FROM_UNIXTIME(ttt.create_time,\"%Y-%m-%d\") create_time,ttt.difficulty_points,t.`name`,su.`name` subject_name,ti.`level`,if(ti.level is null,\"无\",if(ti.`level` = 1,\"A\",if(ti.`level` = 2,\"B\",\"C\"))) level_name,t.`desc`,t.type,ttt.type r_type,ifnull(uttt.`status`,0) type_status")
                    ->group('ttt.id')
                    ->having($having)
                    ->select();
                ...
                foreach ($list as $key => $item){
                    ...
                    // 记录题，需判断是否中断
                    // 获取阶段任务未完成记录第一条
                    $firstInfo = db('user_type_task_log_table')->alias('uttlt')->join('task t','t.id = uttlt.task_id')->where('uttlt.user_type_task_id',$user_type_id)->where('uttlt.status','<>',self::STATUS_ED)->order('uttlt.id asc')->field('uttlt.id,uttlt.done_time,t.type')->find();
                    if(empty($firstInfo)){
                        // 找不到，不管他
                        continue;
                    }
                    if($firstInfo['done_time'] < strtotime(date('Y-m-d')) && $firstInfo['type'] == self::DEFAULT_ED){
                        // 判断执行日期是否小于当前日期，则将状态设置为【-4已中断】
                        $list[$key]['type_status'] = -4;
                    }
                }
        ...
        //校内任务数量
        $xntotal = db('type_task_table')
            ->alias('ttt')
            ->join('type_task_log_table ttlt','ttt.id = ttlt.type_task_id')
            ->join('user_type_task_table uttt',"uttt.type_task_id = ttt.id and uttt.user_id = {$this->info['id']}",'left')
            ->where('ttt.type',self::TYPE_SCHO)
            ->where('ttt.status',self::STATE_OPEN)
            // todo::筛选未取消任务
            ->where('ttlt.is_cancel',self::DEFAULT_NO)
            ->where("(ttt.start_date <= {$endTime} and (uttt.task_over_time = 0 or uttt.task_over_time is null or uttt.task_over_time >= {$startTime})) or (ttt.start_date >= {$startTime} and ttt.start_date <= {$endTime})")
        ...
        //职业体验数量
        $zytotal = db('type_task_table')
            ->alias('ttt')
            ->join('type_task_log_table ttlt','ttt.id = ttlt.type_task_id')
            ->join('user_type_task_table uttt',"uttt.type_task_id = ttt.id and uttt.user_id = {$this->info['id']}",'left')
            ->join('type_task_sign_table ttst',"ttst.type_task_id = ttt.id and ttst.user_id = {$this->info['id']}",'left')
            ->where('ttt.mode',self::MODE_TWO)
            ->where('ttt.type',self::TYPE_JOBS)
            ->where('ttt.status',self::STATE_OPEN)
            // todo::筛选未取消任务
            ->where('ttlt.is_cancel',self::DEFAULT_NO)
            ->where("(ttt.start_date <= {$endTime} and (uttt.task_over_time = 0 or uttt.task_over_time is null or uttt.task_over_time >= {$startTime})) or (ttt.start_date >= {$startTime} and ttt.start_date <= {$endTime})")
        ...
        //公益劳动
        $gytotal = db('type_task_table')
            ->alias('ttt')
            ->join('type_task_log_table ttlt','ttt.id = ttlt.type_task_id')
            ->join('user_type_task_table uttt',"uttt.type_task_id = ttt.id and uttt.user_id = {$this->info['id']}",'left')
            ->join('type_task_sign_table ttst',"ttst.type_task_id = ttt.id and ttst.user_id = {$this->info['id']}",'left')
            ->where('ttt.mode',self::MODE_TWO)
            ->where('ttt.type',self::TYPE_BPUB)
            ->where('ttt.status',self::STATE_OPEN)
            // todo::筛选未取消任务
            ->where('ttlt.is_cancel',self::DEFAULT_NO)
            ->where("(ttt.start_date <= {$endTime} and (uttt.task_over_time = 0 or uttt.task_over_time is null or uttt.task_over_time >= {$startTime})) or (ttt.start_date >= {$startTime} and ttt.start_date <= {$endTime})")
        ...
        $new = [];
        $new[] = $task_data;
        $messageCount = messageCount($this->info['id'],$this->status['ZEROSTATUS']);
        if(empty($list)){
            $list = [];
            $totalPage= 0;
        }else{
            $totalPage = ceil(count($task_data[0])/$limit);
        }
        return $this->returnJson(['messageCount'=>$messageCount,'task_data'=>$new,'list' => $list,'currentPage' => $page,'totalPage' => $totalPage],'获取成功',1);
    }
    ...
    public function signTask() {
        ...
        //         'tasking_time'=>0,
        //         'record_time'=>0,
        //         'task_over_time'=>$info['end_date']
        //     ];
        //     $user_task_id = db('user_type_task_table')->insertGetId($insert_user_type_task_table);
        // } catch (\Throwable $th) {
        //     var_dump($h);
        // }
    ...
    private function synWorkTask($user_task_log_id = 0){
        ...
        $info = db('user_type_task_log_table')
            ...
            ->field('uttlt.id,ttt.type,uttlt.teacher_id,ttt.task_id,uttt.id user_type_task_id')
//                    ->field('uttlt.id,ttt.type,uttlt.sub_ans,uttlt.obj_ans,uttlt.sub_files,uttlt.obj_files,uttlt.teacher_id,ttt.task_id')
        ...
        if($info['type'] == self::TYPE_WORK){
            // todo::20220904 - 判断是否包含未完成阶段任务
            if(!$this->checkIsHasOtherLogs($user_task_log_id)){
                $count = db('user_work_task_log_table')->where('user_type_task_log_id',$user_task_log_id)->count();
                logs($count,'评分情况');
                if($count > 0){
                    // 已经同步，不需要重复同步
                    return true;
                ...
                // 获取所有阶段任务列表
                $list = db('user_type_task_log_table')->where('user_type_task_id',$info['user_type_task_id'])->select();
                $logs = [];
                foreach ($teacher_ids as $teacher_id){
                    foreach ($list as $item){
                        array_push($logs,[
                            'user_type_task_log_id' => $item['id'],
                            'user_type_task_id' => $item['user_type_task_id'],
                            'task_id' => $item['task_id'],
                            'sub_ans' => $item['sub_ans'],
                            'obj_ans' => $item['obj_ans'],
                            'sub_files' => $item['sub_files'],
                            'obj_files' => $item['obj_files'],
                            'teacher_id' => $teacher_id
                        ]);
                    }
                }
...
//           }
        ...
        $taskInfo = db('user_type_task_log_table')
            ...
            ->find();
        // todo::判断当前任务是否提交及时
        if(($status == self::STATUS_BEFORE_N && $now > $taskInfo['end_date']) || ($status == self::STATUS_EVAL_ED && $now > $taskInfo['task_over_time'])){
            // todo::为课前任务时，判断当前时间是否在项目结束时间之后 或者 为课后任务时，判断当前时间是否在课后阶段结束时间之后
            $updates['is_once'] = 2; // 1及时 2不及时
        }
        db()->startTrans();
        try{
    ...
    public function complatedTypeTaskList(){
        ...
        $groupSql = db('user_type_task_table')
            ->field('type_task_id,count(id) had_person_num')
            ->group('type_task_id')
            ->select(false);
        $list = db('user_type_task_table')
            ->alias('uttt')
            ->join('type_task_table ttt',"uttt.type_task_id = ttt.id")
            ->join('task t',"ttt.task_id = t.id")
            ->join("task_index ti",'ti.task_id = t.id','left')
            ->join("subject su",'su.id = ti.subject_id','left')
            ->join("({$groupSql}) uttt1","uttt1.type_task_id = ttt.id")
            ->where($where)
            ->limit(($page-1)*$limit,$limit)
            ->field('su.`name` subject_name,t.`type` q_type,ttt.title,ti.`level` level_name,t.`desc`,t.id task_id,t.name,if(t.image is null,"",if(instr(t.image,"http") > 0,t.image,concat("'.$this->webUrl.'",t.image))) task_image,t.desc task_desc,ttt.id,ttt.title,ttt.depict,if(ttt.image is null,"",if(instr(ttt.image,"http") > 0,ttt.image,concat("'.$this->webUrl.'",ttt.image))) image,ttt.person_num,uttt1.had_person_num,ttt.content,ttt.`mode`,ttt.type,FROM_UNIXTIME(ttt.start_date,"%Y-%m-%d") start_date,FROM_UNIXTIME(ttt.end_date,"%Y-%m-%d") end_date,FROM_UNIXTIME(ttt.create_time,"%Y-%m-%d") create_time,ttt.difficulty_points,uttt.`status` type_status')
    ...
    public function overdueTypeTaskList(){
        $page  = $this->request->param('page', 1);
        $limit = $this->request->param('limit', 10);
        $type = $this->request->param('type ', 14);
        ...
        if(!empty($month)){
            ...
            $cwhere .= " and uttt.task_over_time <> 0 and uttt.task_over_time < {$endTime}";
        }else{
            $cwhere .= " and uttt.task_over_time <> 0 and uttt.task_over_time < {$endTime}";
        }
        // 家庭任务和劳动大赛
        $home_work_task_sql = db('user_type_task_table')
            ...
            ->field('su.`name` subject_name,t.type q_type,ttt.title,ti.`level` level_name,t.`desc`,ttt.`depict`,t.id task_id,t.`name`,if(t.image is null,"",if(instr(t.image,"http") > 0,t.image,concat("'.$this->webUrl.'",t.image))) task_image,t.`desc` task_desc,ttt.id,if(ttt.image is null,"",if(instr(ttt.image,"http") > 0,ttt.image,concat("'.$this->webUrl.'",ttt.image))) image,sttt.end_date_time,ttt.person_num,ttt.content,ttt.`mode`,ttt.type,FROM_UNIXTIME(ttt.start_date,"%Y-%m-%d") start_date,FROM_UNIXTIME(ttt.end_date,"%Y-%m-%d") end_date,FROM_UNIXTIME(ttt.create_time,"%Y-%m-%d") create_time,ttt.difficulty_points,-3 type_status')
            ->select(false);
        // 其余项目
...
//            ->whereNotIn('ttt.type',[self::TYPE_HOME,self::TYPE_WORK])
            ->field('su.`name` subject_name,t.type q_type,ttt.title,ti.`level`,t.`desc`,ttt.`depict`,t.id task_id,t.`name` task_name,if(t.image is null,"",if(instr(t.image,"http") > 0,t.image,concat("'.$this->webUrl.'",t.image))) task_image,t.`desc` task_desc,ttt.id,if(ttt.image is null,"",if(instr(ttt.image,"http") > 0,ttt.image,concat("'.$this->webUrl.'",ttt.image))) image,uttt.task_over_time end_date_time,ttt.person_num,ttt.content,ttt.`mode`,ttt.type,FROM_UNIXTIME(ttt.start_date,"%Y-%m-%d") start_date,FROM_UNIXTIME(uttt.task_over_time,"%Y-%m-%d") end_date,FROM_UNIXTIME(ttt.create_time,"%Y-%m-%d") create_time,ttt.difficulty_points,-3 type_status')
            ->select(false);
    ...
    public function overdueTypeTaskNumber(){
        $page  = $this->request->param('page', 1);
        $limit = $this->request->param('limit', 10);
        $type = $this->request->param('type ', 14);
        $year = $this->request->param('year',date('Y')); // 筛选年
...
//            ->whereNotIn('ttt.type',[self::TYPE_HOME,self::TYPE_WORK])
    ...
    public function week(){
        ...
        list($through_rate,$done_rate,$once_rate,$record_datas) = $this->deal_census($start_time,$end_time);
        return $this->returnJson(compact('through_rate','done_rate','once_rate','record_datas'),'获取成功',1);
    }
    ...
    private function deal_census($start_time = 0,$end_time = 0){
        ...
        $truth_count = db('user_type_task_log_table')
            ...
            ->count('task_id');
        // 4.5 获取完成任务及时数
        $once_count = db('user_type_task_log_table')
            ->where("update_time >= {$start_time} and update_time <= {$end_time}")
            ->where('user_id',$this->info['id'])
            ->where('status',self::STATUS_ED)
            ->where('is_once',1)
        ...
        $list = db('user_type_task_log_table')
            ...
            ->where('uttlt.user_id',$this->info['id'])
            ->field("uttlt.id,CONCAT_WS('-',uttlt.user_type_task_id,uttlt.type,uttlt.task_id) task_id,uttlt.sub_ans,FROM_UNIXTIME(uttlt.done_time,'%Y-%m-%d') done_time")
            ->select();
        ...
        foreach ($task_ids as $task_id){
            ...
            list($user_type_task_id,$type,$t_id) = explode('-',$task_id);
            // 获取当前阶段任务信息
            $ext = db('task_question_log')
                ->alias('tql')
                ->join('question q','q.id = tql.question_id')
                ->where('tql.task_id',$t_id)
                ->value('q.ext');
            if(empty($ext)) return $this->returnJson([],'无效请求');
            $ext = unserialize($ext);
            $doneList = [
                'id' => $task_id,
                'task_name' => db('task')->where('id',$t_id)->value('name'),
        ...
        $once_rate = [
            ...
            'total_count' => $total_count,
            'rate' => empty($total_count)?0:($once_count/$total_count)*100
        ];
        return [$through_rate,$done_rate,$once_rate,$record_datas];
    }
     ...
     * @return \think\response\Json
     */
    private function softTaskList($type = self::TYPE_SOFT){
        $page  = $this->request->param('page', 1);
        $limit = $this->request->param('limit', 10);
        $status = $this->request->param('status',''); // 状态筛选，默认显示全部
